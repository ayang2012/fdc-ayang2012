...
def load(df):
    ...
    restaurant_table = sqlalchemy.Table('restaurants', metadata,
        sqlalchemy.Column('id', sqlalchemy.Integer, primary_key=True),
        sqlalchemy.Column('name', sqlalchemy.String),
    )

    product_table = sqlalchemy.Table('products', metadata,
        sqlalchemy.Column('id', sqlalchemy.Integer, primary_key=True),
        sqlalchemy.Column('name', sqlalchemy.String),
        ...
        sqlalchemy.Column('restaurant_id', sqlalchemy.Integer, ForeignKey('restaurants.id'))
    )
    ...
    restaurant_df = pd.DataFrame(df['restaurant'].unique(), columns=['name']).reset_index().rename(columns={'index':'id'})
    ...
    product_columns = ['id', 'name_x', 'ingredients', 'allergens', 'picture_url', 'category', 'restaurant_id']
    product_df = df.merge(restaurants_tb.rename(columns={'id':'restaurant_id'}), left_on='restaurant', right_on='name')[product_columns]
    product_df.rename(columns={'name_x':'name', 'product_category':'category'}, inplace=True)
